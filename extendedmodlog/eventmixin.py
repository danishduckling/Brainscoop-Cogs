...
class EventMixin:
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        if embed_links:
            ...
            if perp and not (guild.id in self._ban_cache and member.id in self._ban_cache[guild.id]):
                embed.add_field(name=_("Kicked"), value=perp.mention)
            if perp and (guild.id in self._ban_cache and member.id in self._ban_cache[guild.id]):
